...
def search(query: str,
    ...
    result = MinimalSearchResults(question_id="",
                                  question=query,
        ...
        for future in tqdm(as_completed(future_to_q),
            ...
            try:
                ...
                search_results.append(
                    MinimalSearchResults(question_id=q.question_id,
                                         question=q.question,
                                         retrieved_sources=sources)
                )
...
def answer(query: str, k: int = 5, index_dir: str = "data/processed",
    ...
    result = MinimalAnswer(question_id="", question=query,
...
def answer_dataset(student_search_results_path: str, save_directory: str,
    ...
    for res in tqdm(student_data.search_results, desc="Generating answers"):
        ...
        answered_results.append(
            MinimalAnswer(question_id=res.question_id, question=res.question,
                          retrieved_sources=res.retrieved_sources, answer=ans)
        )

    output = StudentSearchResultsAndAnswer(search_results=answered_results, k=student_data.k)
...
def evaluate(student_search_results_path: str, dataset_path: str, k: Optional[int] = None) -> None:
